...
public class AirportServiceAction implements Serializable {
	...
	public void search() {
		...
		try {
			...
			List<Airport> airportsList = rehlatiEJB.callAirportServiceGetAirportsByCountry(getCountry()).getAirportList();
			
			Map<String, Airport> map = new LinkedHashMap<String, Airport>();
			for (Airport ays : airportsList) {
			  map.put(ays.getAirportCode(), ays);
			}
			airportsList.clear();
			airportsList.addAll(map.values());
